...
//	private String schemName;
	...
	public NBSFile(InputStream is) {
		...
		for (Field declaredField : this.getClass().getDeclaredFields()) {
			...
			if (declaredField.getType().isAssignableFrom(short.class)) declaredField.set(this, inputStream.readShort());
			else if (declaredField.getType().isAssignableFrom(String.class))
				try {
					declaredField.set(this, inputStream.readUTF());
				} catch (Exception e) {
					System.err.println("Failed to read string for " + declaredField.getName() + "; defaulted to \"Unknown\"");
					declaredField.set(this, "Unknown");
				}
			else if (declaredField.getType().isAssignableFrom(byte.class)) declaredField.set(this, inputStream.readByte());
			else if (declaredField.getType().isAssignableFrom(int.class)) declaredField.set(this, inputStream.readInt());
